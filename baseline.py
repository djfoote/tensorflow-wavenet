...
def basic_transfer(content_signal, style_signal):
    with tf.Graph().as_default():
        with tf.Session() as sess:
            initial = tf.Variable(tf.random_normal(content_signal.shape) * 0.256)
            style_placeholder = tf.placeholder(tf.float32, shape=style_signal.shape)
            content_placeholder = tf.placeholder(tf.float32, shape=content_signal.shape)

            gram_style = tf.matmul(tf.transpose(style_placeholder), style_placeholder)
            gram_initial = tf.matmul(tf.transpose(initial), initial) 
            # content_tensor = tf.Variable(content_placeholder)
            content_loss = tf.nn.l2_loss(initial - content_placeholder) / content_placeholder.get_shape()[0].value
            style_loss = tf.nn.l2_loss(gram_initial - gram_style) / gram_style.get_shape()[0].value**2
            # print(style_gram.size)
            loss = 1e-2 * content_loss
            train_step = tf.train.AdamOptimizer(LEARNING_RATE).minimize(loss, var_list = [initial])
            tf.initialize_all_variables().run()
            temp1 = tf.convert_to_tensor(style_signal)
            temp2 = tf.convert_to_tensor(content_signal)
            print(temp1.get_shape(), temp2.get_shape())
            for i in np.arange(5000):
                sess.run([train_step], 
                    feed_dict={style_placeholder:content_signal, content_placeholder: style_signal})
                if i % 1000 == 0:
                    print("iteration number: ", i)
                    _, curr_loss = sess.run([train_step, loss], 
                    feed_dict={style_placeholder:content_signal, content_placeholder: style_signal})
                    print("loss at:", curr_loss)
            generate.write_wav(initial.eval(), SAMPLE_RATE, "firs_try.wav")
...
def main():
    ...
    print(len(content_signal), len(style_signal))
    print (content_signal, style_signal)
    content_signal = np.reshape(content_signal[:100000],(-1,1)) 
    style_signal = np.reshape(style_signal[:100000], (-1,1))
    basic_transfer(
